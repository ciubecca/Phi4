# This file contains various core functions related to matrix evaluation, which have 
# been moved to a Cython module for performance, since they contain the most 
# intensive loops.
...
# Precomputed symmetry factors related to the action of an operator on a state
...
def yieldBasis(basis, idx, EL, helper, nd, nc, dlistPos, oscEnergies, oscList):
    """ This efficiently computes a set of states which are obtained by acting 
    on a state in the basis by the action of a set of oscillators.
    It can be used both to efficiently compute perturbative contribution in the 
    old fashioned PT, and for a potential future implementation of the NLO formalism
    basis: basis of states which is acted upon
    idx: index of state in the basis
    EL: maximal energy of states to generate
    helper: Helper instance
    nd, nc: number of annihilation and creation operators 
    dlistPos, oscEnergies, oscList: precomputed data structures related to the 
    action of the operators on the state
    """
...
def computeME(basis, i, destbasis, ignKeyErr, nd, nc, dlistPos, 
        ...
        destbasis: basis containing states which are generated by the action of the operators.
            When ignKeyErr is False and a state is generated which is not found in destbasis,
        ...
        ignKeyErr: this must be set to True if the action of an oscillators on an input state
            can generate a state not in lookupbasis. This should be True when evaluating old fashioned 
            PT contributions, or when implementing the NLO formalism. Otherwise it should be set to False
        nd, nc: number of annihilation and creation operators 
        dlistPos, oscFactors, oscList, oscEnergies: precomputed data structures related to the 
            action of the operators on the state
        """
