...
# Normal prescription for comparing ET and LC data, expressing the data in LC coupling
...
# Equal time mass gap rescaled to units with meff=1
    massrawlistResc = massrawlist/LCmassEff
    massrenlistResc = massrenlist/LCmassEff


    plt.figure(1)
    plt.plot(gLCeffNorm, massrawlistResc, label="ET raw")
    plt.plot(gLCeffNorm, massrenlistResc, label="ET ren")
    plt.plot(gLCeffNorm, gapLC, label=r"$LC, \Delta_{\rm max}=34$")
    plt.plot(gLCeffNorm, gapLCExtr, label=r"$LC, \Delta_{\rm max}=\infty$")
    # plt.plot(glist, gapLCnaive, label="LC naive")
    plt.plot(gLCeffNorm, mpert(glist)/LCmassEff, label=r"$o(g^3)$", color='y', linestyle="--", marker='o', markersize=1)

    # plt.plot([1], gapLCvevextrg1Resc, 'ok')

    plt.xlim(0, max(gLCeffNorm)+0.01)
    # plt.ylim(0,1)
    ...
    s = "gapvsG_prescr1_ET={}_L={}_v2".format(ET,L)
