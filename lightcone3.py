...
def main():
    ...
    # Effective ET coupling (in units where bare mass=1)
    ...
    gapET = np.array([E1ren[g] - E0ren[g] for g in gETeff])

    # Scale the equal time gap to appropriate units
    gapETnorm = gapET/np.sqrt(meffsqList)
...
# Light cone mass gap
    gapLC = np.sqrt(msqLClist2)


    plt.figure(1)
    plt.plot(gLClist, gapETnorm, label="ET")
    plt.plot(gLClist, gapLC, label="LC")
