...
#    @profile
    def buildBasisAndMatrix(self,L,ET,m):
        ...
        V31factor = {k4: [f3([x[1] for x in state])/(4*math.sqrt(Omega[k4]*np.prod([Omega[n]**Zn for (n,Zn) in state])))
                               for state in V31[k4]] for k4 in list(V31.keys())}
        V31energy = {k4: [energy(state,L,m)- Omega[k4] for state in V31[k4]] for k4 in list(V31.keys())}
        ...
        def symfactor(xxx_todo_changeme):
            (k3,k4) = xxx_todo_changeme
            if k3==k4:
                 return 1
            else:
                 return 2
        ...
        V22factor = {key: [symfactor(key)*f2([x[1] for x in state])/
                                ...
                                for key in list(V22.keys())}
        V22energy = {key: [energy(state,L,m) - Omega[key[0]] - Omega[key[1]] for state in V22[key]] for key in list(V22.keys())}
