...
for ET in ETlist:
    ...
    for L in Llist:
        ...
        phi01.append(2/L*np.inner(a.eigenvectors[glist[0]]["renloc"][0], a.V[1].dot(b.eigenvectors[glist[0]]["renloc"][0]))**2)
    ...
    plt.figure(2)
    plt.plot(Llist, Gap, label="Emax={}".format(ET))
...
plt.xlim(min(Llist),max(Llist))
plt.xlabel("L")
plt.ylabel(r"$M_{\rm gap}$")
s = "MasscheckvsL_g={}".format(g)
plt.title(r"$g$ = {}".format(g))
plt.legend()
fname = ".{0}".format(output)
# plt.savefig(s+fname, bbox_inches='tight')
